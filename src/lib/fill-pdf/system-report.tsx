...
export const generateSystemReport = async ({
  ...
  // Center alignment for text lines
  const title = "System Overview Report";
  const dateRangeText =
    date?.from && date?.to
  ...
  doc.text("System Key Performance Indicators", margin, kpiY);
  kpiY += 10;

  try {
    // Capture the KPI Cards
    const kpiImage = await captureComponent(SystemKPICards, {
      data,
      dateRange: date,
      requestType,
      exporting: true,
