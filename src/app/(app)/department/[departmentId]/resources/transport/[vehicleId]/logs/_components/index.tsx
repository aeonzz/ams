...
import VehicleAuditLogTable from "./vehicle-audit-log-table";
import { ChevronLeft, Search } from "lucide-react";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { useRouter } from "next/navigation";
import VehicleAuditLogSkeleton from "./vehicle-audit-log-skeleton";
...
export default function VehicleLogsScreen({ vehicleId }: VehicleLogsScreenProps) {
  ...
  >({
    queryFn: async () => {
      const res = await axios.get(`/api/audit-log/vehicle-log/${vehicleId}`);
    ...
    <div className="flex h-full w-full flex-col">
      <div className="flex h-[50px] items-center justify-between border-b px-3">
        <div className="flex items-center gap-1">
          ...
          <H5 className="truncate font-semibold">Vehicle Logs</H5>
        </div>
        <SearchInput />
      </div>
